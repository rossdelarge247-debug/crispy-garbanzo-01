...
export default function ErrorState({ title, description, onRetry, retryLabel }: ErrorStateProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-6 text-center border-2 border-dashed border-conviction-danger/30">
      {/* Big X */}
      <div className="text-4xl font-black text-conviction-danger mb-4">✕</div>
      ...
      {onRetry && (
        <button
          onClick={onRetry}
          className="inline-flex items-center px-5 py-2.5 border-2 border-black text-black text-sm font-black uppercase tracking-widest hover:bg-black hover:text-white transition-colors"
        >
