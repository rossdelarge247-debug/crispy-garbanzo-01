        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <span className="text-xs font-semibold text-[--text]">{s.entryDate} → {s.exitDate}</span>
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          <h1 className="text-xl font-bold text-[--text]">{humanName}</h1>
          {rec && (
        ...
        {entryPrice > 0 && <span className="text-lg font-bold tabular-nums text-[--text]">{fp(entryPrice)}</span>}
      </div>

      {/* Confidence */}
      {rec && (
        <div className="rounded-lg card p-4 mb-4">
      ...
      <div className="rounded-lg card p-4 mb-4">
        ...
        {topHyp && (
          <div className="pl-3 border-l-2 border-[--accent]/30">
            <p className="text-xs font-medium text-[--text]">{topHyp.title}</p>
      ...
      {/* Trade plan */}
      {rec && (
        <div className={`rounded-lg p-4 mb-4 ${rec.action === "enter_now" ? "bg-[--green-bg]" : rec.action === "wait" ? "bg-[--amber-bg]" : "card"}`}>
          ...
          {rec.entryPrice > 0 ? (
            <div className="grid grid-cols-4 gap-3 text-xs mb-3">
              <div><p className="text-[--text-muted]">Entry</p><p className="font-bold tabular-nums text-[--text]">{fp(rec.entryPrice)}</p></div>
              ...
              <div><p className="text-[--text-muted]">Hold</p><p className="font-bold text-[--text]">{rec.holdDays}d</p></div>
            </div>
          ) : rec.entryText ? (
            <div className="text-xs space-y-0.5 mb-3">
              <p>Entry: <span className="font-medium text-[--text]">{rec.entryText}</span></p>
              ...
              {rec.holdText && <p>Hold: <span className="font-medium text-[--text]">{rec.holdText}</span></p>}
            </div>
          ) : null}
          ...
          <div className="flex gap-2">
            ...
            <Link
              ...
              className="flex-1 px-3 py-2 text-xs font-semibold rounded bg-[--surface-overlay] text-[--text-secondary] text-center hover:text-[--text] transition-colors"
            >
              Full trade plan
            </Link>
          </div>
        ...
        <div className="mb-4">
          ...
          {hypotheses.map(h => (
            <div key={h.id} className="text-xs pb-2 mb-2 last:mb-0 last:pb-0">
              ...
              <span className="font-semibold text-[--text] ml-1">{h.title}</span>
