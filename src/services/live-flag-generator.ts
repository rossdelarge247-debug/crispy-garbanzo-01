...
function buildHypotheses(
  ...
  if (aiAnalysis?.source === "ai" && aiAnalysis.scenarios.length > 0) {
    return aiAnalysis.scenarios.map((scenario, i) => ({
      ...
      summary: `${scenario.consequence || scenario.priceImpact}`,
      rationale: `Trigger: ${scenario.trigger}. Price impact: ${scenario.priceImpact}. Timeframe: ${scenario.timeframe}. Based on analysis of ${matchedArticles.length} recent articles.`,
